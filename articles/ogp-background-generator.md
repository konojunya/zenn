...
`0xjj.dev` ではブログ記事ごとにパステルカラーのふわっとした OGP 背景画像を自動生成しています。同じ記事に対して何度実行しても同一の画像が出力される「冪等性」を持たせているのがこだわりポイントで、今回はその Go 実装を解説していきます。
...
ここで `math/rand` を使っていないのもポイントで、Go の標準ライブラリのランダム実装はバージョンによってアルゴリズムも変わる可能性があります。自前のハッシュにしておけば Go のバージョンが変わっても同じ出力を保証できます。
...
ノイズには Quintic（5 次）補間によるバリューノイズを使っています。
...
半径の違いでどれくらい仕上がりが変わるかを見てみます。ブラーの効果を分かりやすくするために、ガウシアン減衰を使わずフラットな円（ステップ関数）で blob を描画した画像にブラーをかけてみます。
...
uint8 のまま計算すると桁落ちや丸め誤差が蓄積して、ブラー 3 パスを経た結果にバンディング（色の段差）が出てしまいます。float64 で演算し、最後は uint8 へ変換することで、滑らかなグラデーションを維持できます。
